...
class SharedBLSTM(nn.Module):
    """Shared Bidirectional LSTM encoder for question and answer."""
...
class ClassicalAttention(nn.Module):
    """Classical Self-Attention for the Question."""
...
class PositionalAttention(nn.Module):
    """Positional Attention Layer modulated by Gaussian influence."""
...
# ── 4. Main Model (RNN-POA) ──
class RNNPOA(nn.Module):
    """Full Positional Attention Based RNN Model."""
...
class AttentionBLSTM(nn.Module):
    """Baseline: RNN-ATT (Classical attention-based BLSTM without positional info)."""
    def __init__(self, embed_matrix, hidden_dim=50, dropout=0.2):
        super().__init__()
        self.encoder = SharedBLSTM(embed_matrix, hidden_dim, dropout)
